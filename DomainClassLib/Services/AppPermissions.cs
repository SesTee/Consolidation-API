...
using DomainClassLib.Data.Services;
...
namespace DomainClassLib.Services
{
    public class AppPermissions : IAppPermissions
    {
        ...
        public async Task<bool> SaveRefreshTokenAsync(string refreshtoken, string token)
        {
            try
            {
                var result1 = await _context.TokenTbl.AddAsync(new Data.Entities.AppPerm.TokenTbl { Refresh = refreshtoken, Token = token, DateCreated = DateTime.Now });
                await _context.SaveChangesAsync();
            }
            catch (Exception ex)
            {
